...
/* Greg Ziegan
 * grz5
 * Working on previews for a long time, was not implemented but took many hours along with BuggedSS
 * Previews would be a dummy tower without a script to see when scrolling over grid pieces
 */ 

public class BuggedGM : MonoBehaviour {
	...
	void Start () {
		...
		for (int i = -19; i < 20; i++) {
			for (int j = -14; j < 0; j++) {
				gridPiece = Instantiate(gridPrefab, new Vector3(i, 3, j), Quaternion.identity) as GameObject;
	...
	void OnGUI(){
		...
		if (tSelected) {
			selectedTower.RangeObject.renderer.enabled = true;
		}
	...
	void setSelectedTower(Tower sTower) {
		Debug.Log ("SetSelectedTower called");
		tSelected = true;
		if (selectedTower)
			selectedTower.RangeObject.renderer.enabled = false; 
